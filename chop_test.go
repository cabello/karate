...
func TestAllChoppers(t *testing.T) {
	choppers := []func(haystack []int) Chopper{
		ChannelChopper,
		IterativeChopper,
		RecursiveChopper,
		TraditionalChopper,
...
func testElementNotFound(t *testing.T, chopper func(haystack []int) Chopper) {
...
func testElementFoundExtremeLeft(t *testing.T, chopper func(haystack []int) Chopper) {
	assertChop(t, 0, chopper, 1, []int{1})
	assertChop(t, 0, chopper, 1, []int{1, 3, 5})
	assertChop(t, 0, chopper, 1, []int{1, 3, 5, 7})
}

func testElementFoundExtremeRight(t *testing.T, chopper func(haystack []int) Chopper) {
	assertChop(t, 2, chopper, 5, []int{1, 3, 5})
	assertChop(t, 3, chopper, 7, []int{1, 3, 5, 7})
}

func testElementFoundAnywhere(t *testing.T, chopper func(haystack []int) Chopper) {
	assertChop(t, 1, chopper, 3, []int{1, 3, 5})
	assertChop(t, 1, chopper, 3, []int{1, 3, 5, 7})
	assertChop(t, 2, chopper, 5, []int{1, 3, 5, 7})
}

func testElementFirstRightSecondLeft(t *testing.T, chopper func(haystack []int) Chopper) {
	assertChop(t, 7, chopper, 12, []int{1, 3, 4, 5, 7, 9, 10, 12, 14, 15, 16})
}

func assertChop(t *testing.T, expected int, chopper func(haystack []int) Chopper, needle int, haystack []int) {
	c := chopper(haystack)

	if result := c.Chop(needle); result != expected {
